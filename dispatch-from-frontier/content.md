...
By default Claude will ask for permission to perform certain actions like reading from a directory, editing a file, running bash commands, etc. You can bypass these asks and let Claude do whatever it wants by running in a "headless" mode. This can be enabled by launching Claude Code with the `--dangerously-skip-permissions` flag set. I use this this feature to let Claude work uninterrupted.

One more note about permissions: since this experiment involves letting Claude use tools unfettered, I found it crucial to set up [proper hooks](https://code.claude.com/docs/en/hooks-guide) that restrict Claude from performing destructive actions on my machine. Hooks are small pieces of code that run at certain points within the agentic loop. These protective hooks do things like prevent Claude from running `rm -rf` on my root directory and the like. Another example is a hook that prevents Claude from editing its own hooks! To illustrate the importance of this: imagine that Claude decides to remove your root directory, sees it's blocked from doing so by a hook, and then edits its own config files to remove that protective hook entirely. Claude would be free to delete your root folder. Even with hooks, you should be wary about using this "dangerous" mode on your personal computer. If you do this, I recommend setting up some kind of walled off sandbox environment separate from your personal files.
...
You can find the completed project on GitHub here: https://github.com/enerrio/gitgym
...
I've uploaded the full transcripts of my chats in a separate repo: https://github.com/enerrio/gitgym-prompts. Be sure to check it out for more info.
...
Eventually Claude had completed all the tasks and I was happy with the results. The tool had worked as expected but I did have some concerns and notes.

## Finishing Touches
I went through all the exercises to make sure everything was working well. I like the exercises. They were educational and accurate, and I ended up learning some things I didn't know before. I do think it is a genuinely useful hands-on intro to git. 

There were a few minor things that I prompted Opus to fix like a more comprehensive README, confusing exercise names, and an extra command to clean the workspace so the exercises don't pollute a user's machine. Opus made these changes and also helped walk me through the process of setting up CI through GitHub Actions and publishing the package on PyPI (I had not done this before so it was very educational for me as well).
